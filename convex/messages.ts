    ...
    try {
      ...
      for (const message of visiblePage) {
        ...
        enhancedMessages.push({
          id: message.id ?? "",
          role: (message.role === "tool" ? "system" : message.role) as "user" | "assistant" | "system",
          parts: message.parts ?? [],
    ...
    try {
      ...
      for (const msg of visibleMessageResults) {
        ...
        combinedResults.push({
          ...
          updated_at: chat?.update_time || msg.update_time || msg._creationTime,
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const frozenMessages = messages.filter(
        (msg) => (msg.update_time ?? msg._creationTime) <= chat.share_date! && msg.is_hidden !== true,
      );
      ...
      return frozenMessages.map((msg) => ({
        ...
        content: msg.content,
        update_time: msg.update_time ?? msg._creationTime,
