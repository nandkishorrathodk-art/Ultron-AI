...
export const DESKTOP_UPDATE_URL =
  "https://github.com/Ultron-AI-tech/Ultron-AI/releases/latest";
...
async function promptDesktopUpdate(): Promise<void> {
  toast.error("Update Ultron-AI Desktop to sign in", {
    description:
...
export async function navigateToAuth(
  ...
  if (detectTauri()) {
    try {
      ...
      // redirects to localhost instead of the Ultron-AI:// deep link
      try {
...
export async function saveFileToLocal(
  ...
  const delimiter = `Ultron-AI_EOF_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`;
