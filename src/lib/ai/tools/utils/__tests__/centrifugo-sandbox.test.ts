...
describe("CentrifugoSandbox", () => {
  ...
  describe("files.write", () => {
    it("uses heredoc approach for text content", async () => {
      ...
      try {
        const sandbox = createSandbox();
        await sandbox.files.write("/tmp/Ultron-AI/test.txt", "hello world");
        ...
        expect(writeCmd.command).toContain("<<'Ultron-AI_EOF_");
  ...
  describe("git-bash on Windows", () => {
    ...
    it("downloadFromUrl emits POSIX mkdir + curl with MSYS paths", async () => {
      ...
      await sandbox.files.downloadFromUrl(
        ...
        "/tmp/Ultron-AI-upload/image.png",
      );
      const cmd = runs[0];
      expect(cmd).toContain("mkdir -p '/c/temp/Ultron-AI-upload'");
      ...
      expect(cmd).toContain("-o '/c/temp/Ultron-AI-upload/image.png'");
    ...
    it("files.write for text content uses heredoc with MSYS path", async () => {
      ...
      expect(runs[1]).toContain("<<'Ultron-AI_EOF_");
