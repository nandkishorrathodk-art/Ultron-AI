...
export function getUserFacingE2BErrorMessage(error: unknown): string | null {
  ...
  if (error instanceof AuthenticationError) {
    return "Sandbox authentication failed. The E2B API key may be invalid or expired. Please contact Ultron-AI support.";
  }
  ...
  if (error instanceof TemplateError) {
    return "Sandbox template is incompatible. Please contact Ultron-AI support.";
  }
  if (error instanceof TimeoutError) {
  ...
  if (error instanceof InvalidArgumentError) {
    return "Invalid sandbox configuration. Please contact Ultron-AI support.";
  }
