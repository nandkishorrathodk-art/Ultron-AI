...
async function doEnsureCaido(
  ...
  const createB64 = Buffer.from(
    JSON.stringify({
      query:
        'mutation { createProject(input: {name: "Ultron-AI", temporary: true}) { project { id } error { ... on NameTakenUserError { code } ... on PermissionDeniedUserError { code } ... on OtherUserError { code } } } }',
    }),
  ).toString("base64");
  ...
  const script = [
    ...
    ``,
    `# Find or create the "Ultron-AI" project`,
    ...
    `PROJECT_ID=$(echo "$PROJECTS" | grep -o '"id":"[^"]*","name":"Ultron-AI"' | grep -Eo '"id":"[^"]*"' | cut -d'"' -f4 || echo "")`,
